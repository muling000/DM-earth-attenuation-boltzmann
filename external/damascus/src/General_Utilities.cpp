...
//Read config file and define Input Parameters. Also calculate fractional days, earth velocity, cutoff velocity and set up the detector.
	void Read_Config_File(const char *inputfile)
	{
		...
		//Earth model (optional for backwards compatibility)
			EarthModel="PREM";
			cfg.lookupValue("earthmodel",EarthModel);
			if(EarthModel!="PREM"&&EarthModel!="BoltzmannBenchmark")
			{
				cerr<<"Earth model option \"" <<EarthModel <<"\" is not a valid option." <<endl;
				exit(EXIT_FAILURE);
			}
		//Form Factor
			try
			...
			if(EarthModel=="BoltzmannBenchmark"&&FormFactor!="None")
			{
				cerr<<"BoltzmannBenchmark requires formfactor = \"None\"." <<endl;
				exit(EXIT_FAILURE);
			}
		//Initial SampleSize
			try
		...
		//Earth Velocity
			...
			{
				vEarth=Eigen::Vector3d(0.0,240.0*km/sec,0.0);
			}
