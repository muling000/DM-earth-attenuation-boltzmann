...
//Earth Composition
	...
	const int Benchmark_Element_Count=8;
	const double Benchmark_Elements[Benchmark_Element_Count][3]=
	{
		{8,16,	0.466},		//Oxygen		O
		{14,28,	0.277},		//Silicon		Si
		{13,27,	0.081},		//Aluminium		Al
		{26,56,	0.050},		//Iron			Fe
		{20,40,	0.036},		//Calcium		Ca
		{19,39,	0.028},		//Potassium		K
		{11,23,	0.026},		//Sodium		Na
		{12,24,	0.021}		//Magnesium		Mg
	};
	const double Benchmark_Earth_Density=2.7*gram*pow(cm,-3);
...
//Probability to scatter on a certain element. These are calculated in Initialize_PREM().
	...
	double Benchmark_Scatter_Probability[Benchmark_Element_Count][3];
	double Benchmark_Inverse_MFP=0.0;
...
//calculates the space-independent prefactor and the scatternucleus probabilities for a given parameter point
	...
	{
		if(EarthModel=="BoltzmannBenchmark")
		{
			Benchmark_Inverse_MFP=0.0;
			for(int i=0;i<Benchmark_Element_Count;i++)
			{
				double A=Benchmark_Elements[i][1];
				double rate=Benchmark_Elements[i][2]*Benchmark_Earth_Density/NucleusMass(A)*sigmaSI(mX,sigma0,A);
				Benchmark_Scatter_Probability[i][0]=Benchmark_Elements[i][0];
				Benchmark_Scatter_Probability[i][1]=A;
				Benchmark_Scatter_Probability[i][2]=rate;
				Benchmark_Inverse_MFP+=rate;
			}
			if(Benchmark_Inverse_MFP<=0.0)
			{
				cerr<<"BoltzmannBenchmark inverse mean free path is not positive." <<endl;
				exit(EXIT_FAILURE);
			}
			for(int i=0;i<Benchmark_Element_Count;i++)
			{
				Benchmark_Scatter_Probability[i][2]/=Benchmark_Inverse_MFP;
			}
			cout<<std::scientific<<std::setprecision(12)
				<<"BoltzmannBenchmark: sigma_chiN = "<<sigma0/pb
				<<" pb, mean free path = "<<1.0/Benchmark_Inverse_MFP/km
				<<" km." <<std::defaultfloat<<endl;
			return;
		}
		g_PREM[0]=0.0;
		g_PREM[1]=0.0;
	...
	double g_Factor(int layer)
	{
		...
		else if (layer<=9) return g_PREM[1];
		else return 0.0;
	}
...
//Free Path Vector
	...
	Eigen::Vector3d FreePathVector(double mX,double sigma,Eigen::Vector3d& position,Eigen::Vector3d& vel,std::mt19937& PRNG)
	{
		if(EarthModel=="BoltzmannBenchmark")
		{
			Eigen::Vector3d direction=vel.normalized();
			double xi=ProbabilitySample(PRNG);
			double freePath=-log(1.0-xi)/Benchmark_Inverse_MFP;
			double projection=position.dot(direction);
			double discriminant=projection*projection+rEarth*rEarth-position.squaredNorm();
			double distanceToExit=-projection+sqrt(fmax(0.0,discriminant));
			if(freePath<distanceToExit)
			{
				return freePath*direction;
			}
			return (distanceToExit+mm)*direction;
		}
...
//Which nucleus does the DM scatter on?
	 int ScatterNucleus(Eigen::Vector3d& position,std::mt19937& PRNG)
	{
		...
		if(EarthModel=="BoltzmannBenchmark")
		{
			for(int i=0;i<Benchmark_Element_Count;i++)
			{
				sum+=Benchmark_Scatter_Probability[i][2];
				if(sum>xi)
				{
					return Benchmark_Scatter_Probability[i][1];
				}
			}
			return Benchmark_Scatter_Probability[Benchmark_Element_Count-1][1];
		}
